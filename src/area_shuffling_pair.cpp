...
int main(int argc, char *argv[])
{
	...
	strcpy(filearea_const_for, argv[4]);
	strcpy(filearea_const_back_base, argv[5]);
	int ngen = atoi(argv[6]);// 4islo iteraciy	
	strcpy(fileo, argv[7]);
	strcpy(fileod, argv[8]);	

	if ((in_for = fopen(filearea_const_for, "rt")) == NULL)
	{
		printf("Input file %s can't be opened!\n", filearea_const_for);
		return -1;
	}
	int n_for = 0;
	while (fgets(str, sizeof(str), in_for) != NULL)
	{
		DelChar(str, '\n');
		int hlen = strlen(str);
		if(hlen>5)n_for++;
	...
	rewind(in_for);
	char **id_for;
	id_for = new char*[n_for];
	if (id_for == NULL) { printf("Not  enough memory!"); return -1; };
	for (j = 0; j < n_for; j++)
	{
		id_for[j] = new char[30];
		if (id_for[j] == NULL) { printf("Not  enough memory!"); return -1; };		
	}
	i = 0;
	while (fgets(str, sizeof(str), in_for) != NULL)
	{
		...
		for (i = 0; i < nchr; i++)
		{
			...
			while (fgets(str, sizeof(str), in_const_back[i]) != NULL)
			{
				if (*str != '\n')
				{
					...
					for (k = 0; k < n_for; k++)
					{
						if (strstr(str, id_for[k]) != NULL)
						{
							gom = 0;
							break;
						}
					}
					string_area_const[gom][i]++;
				}
			}
	...
	{		
		...
		fclose(in_var[i]);
		qsort(area_var[i], k, sizeof(area_var[i][0]), compare_areas);
		int end = 0;
		for (j = 0; j < k; j++)
		{
			int sta = area_var[i][j].lev;
			spacer[i][j] = sta - end - 1;
			end = area_var[i][j].pra;
			arealen[i][j] = end - sta;
		}
		spacer[i][k] = chr_len[i] - end;
	}
	//**len_count
	int string_area_var_len = 0, string_area_const_len[2] = { 0,0 };
	{
		for (i = 0; i < nchr; i++)
		{
			...
			while (fgets(str, sizeof(str), in_const_back[i]) != NULL)
			{
				...
				one.get_copy(&area_const[j][i][kk[j]]);				
				kk[j]++;
			}
...
//		if (err_min > err_min_thr && n_cyc[j] > n_cyc_thr)break;		
	...
	for (j = 0; j < 2; j++)
	{
		...
		if(j==0)fprintf(outd, "%s_%s_%f", filearea_var_base, filearea_const_for, real[j]);
	...
	fclose(out); 
	for (i = 0; i < n_for; i++)delete[] id_for[i];
	delete[] id_for;
